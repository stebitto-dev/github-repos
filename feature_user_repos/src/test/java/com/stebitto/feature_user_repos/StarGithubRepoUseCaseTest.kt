...
class StarGithubRepoUseCaseTest {
    ...
    fun setup() {
        ...
        starGithubRepoUseCase = StarGithubRepoUseCaseImpl(githubRepository)
    }

    @Test
    fun `invoke should return success result`() = runTest {
        Mockito.`when`(githubRepository.starRepo("owner", "repoName")).thenReturn(Result.success(Unit))
        Mockito.`when`(githubRepository.getUserRepoByName("owner", "repoName")).thenReturn(Result.success(userRepoDetailDTO))
        val result = starGithubRepoUseCase("owner", "repoName")
    ...
    fun `invoke should return failure result if star repo fails`() = runTest {
        Mockito.`when`(githubRepository.starRepo("owner", "repoName")).thenReturn(Result.failure(RuntimeException()))
        val result = starGithubRepoUseCase("owner", "repoName")
